...
RECONSTRUCCION DE ESTADOS ANCESTRALES EN CARACTERES CONTINUOS
...
Esto último es importante porque los estados de caracter ancestal normalmente se estiman un alto grado de incertidumbre (Losos 1999).
...
Vamos a comparar cuatro modelos diferentes: ER, ARD e irreversibles
...
Vamos a hacer una reconstrucción usando la función corHMM de corHM. 
...
Una alternativa cada vez más popular al calculo de las probabilidades marginales es el mapeo estocático. En este caso se usa MCMC para muestrear historias evolutivas de nuestro caracter de interés a partir de su probabilidad a posteriori (Huelsenbeck et al., 2033).
Usamos un modelos evolutivo de tipo Mk como con el cálculo de las probablidades margionales pero en este caso muestreamos historias evolutivas en el árbol para nuestro caracter de interés. Y calculamos la probabilidad a posteriori de nuestro caracter en los nodos en función de frecuciencia que muestra de uno un otro estado de caracter en las multiples mapeos.
...
Y vez de hacer el mapeo a partir del maximum likelihood del modelo ER (como hemos hecho antes) vamos a hacer los mapeos a partir del 95% del intervalo de confianza mediante el método Bayesiano MCMC para muestrear q (tasa de transición de ER) a partir de su distribución a posteriori de porobabilidades.
...
Finalmente phytools nos otra posibilidad para visualizar los resultados.
